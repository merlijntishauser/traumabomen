...
### Two accents, two roles

The system has **two accent colors with distinct jobs.** This is a deliberate split, not a redundancy.

- **`--color-accent` — the brand green.** Used for *content* and *brand identity*: the leaf logomark, the wordmark, tree-edge "default" color, ambient hover gradients on the workspace canvas, soft tints behind cards. **Stays green in both themes** so the brand reads consistently.
- **`--color-action` — the interactive accent.** Used for *interaction*: primary buttons, links, focused input borders, focus rings, checkbox-checked state, segmented-toggle active state, the SELECTED ring around a node. **Green in dark theme, indigo (`#4f46e5`) in light theme.**

Why split them? On warm cream (`#f7f5f2`), green-on-green for buttons feels generic-wellness-app. Indigo on cream reads editorial, literary, "thinking tool" — closer to the *"reflection, not therapy"* voice. The forest stays the forest (content stays green); the *actions* feel calm and considered. Dark theme keeps green for both because the dark forest carries the editorial weight on its own.

Each role exposes the same five tokens: `--color-{accent|action}`, `-hover`, `-subtle`, `-focus-ring`, `-border`. When in doubt: **anything the user *does* uses `--color-action`. Anything that *represents the brand or a tree* uses `--color-accent`.**

### Hover & press

- **Buttons hover**: background shifts to a warmer tier (`--color-bg-hover`) or to the action's hover variant (`--color-action-hover`, ~10% darker). Border may shift up one tier.
- **Links hover**: color from `--color-action` to `--color-action-hover`.
- **Tree-list-item hover**: row tinted with `--color-bg-hover`, link text becomes the action color.
- **No press shrink, no scale transform.** The interface is calm.
- **Focus**: border becomes `--color-action` and a 2px focus ring in `--color-action-focus-ring` outside it.
