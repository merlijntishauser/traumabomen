...
export function renderClassificationStrips(
  ...
  return classifications.map((cls, stripIdx) => {
    const clsColor =
          ...
          return (
            <React.Fragment key={`${cls.id}-p${pi}`}>
              ...
              {showMarkerLabels &&
                pi === 0 &&
                (() => {
                  const lbl = orientation.stripLabelAt(startPos, `cs:${cls.id}`);
                  return (
                    <text
                      x={lbl.x}
                      y={lbl.y}
            ...
            return (
              <g transform={orientation.markerTransform(primaryPos)}>
                <path
                  ...
                  onMouseLeave={hideTooltip}
                />
                {renderSelectionRing(mx, my, isClsSelected)}
                {renderPatternRings(mx, my, `classification:${cls.id}`, patternRings)}
                {showMarkerLabels &&
                  cls.diagnosis_year !== cls.periods[0]?.start_year &&
                  (() => {
                    const lbl = orientation.diagLabelAt(cls.diagnosis_year!, `ct:${cls.id}`);
                    return (
                      <text
                        x={lbl.x}
                        y={lbl.y}
    ...
    return (
      ...
      >
        <circle
          ...
          onMouseLeave={hideTooltip}
        />
        {renderSelectionRing(mx, my, isEntitySelected)}
        {renderPatternRings(mx, my, `trauma_event:${event.id}`, patternRings)}
        {showMarkerLabels &&
          (() => {
            const lbl = orientation.markerLabelAt(year, `t:${event.id}`);
            return (
  ...
  return turningPoints.map((tp) => {
    ...
    return (
      ...
      >
        ...
        />
        {renderSelectionRing(mx, my, isEntitySelected)}
        {renderPatternRings(mx, my, `turning_point:${tp.id}`, patternRings)}
        {showMarkerLabels &&
          (() => {
            const lbl = orientation.markerLabelAt(year, `tp:${tp.id}`);
            return (
              <text x={lbl.x} y={lbl.y} className="tl-marker-label" textAnchor={lbl.textAnchor}>
                {tp.title}
              </text>
  ...
  return lifeEvents.map((le) => {
    ...
    return (
      ...
      >
        ...
        />
        {renderSelectionRing(mx, my, isEntitySelected)}
        {renderPatternRings(mx, my, `life_event:${le.id}`, patternRings)}
        {showMarkerLabels &&
          (() => {
            const lbl = orientation.markerLabelAt(year, `l:${le.id}`);
            return (
              <text x={lbl.x} y={lbl.y} className="tl-marker-label" textAnchor={lbl.textAnchor}>
                {le.title}
              </text>
