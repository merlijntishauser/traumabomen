...
ALLOWED_STORAGE="traumabomen-theme|traumabomen-canvas-settings|traumabomen-mobile-dismissed|traumabomen_access_token|traumabomen_refresh_token|traumabomen_onboarding_acknowledged|traumabomen_welcome_dismissed|mentalHealthBannerDismissed|i18nextLng|i18next|STORAGE_KEY|TOKEN_KEY|REFRESH_KEY|ONBOARDING_KEY|WELCOME_DISMISSED_KEY|test-settings"

# Find localStorage.setItem calls that don't use allowlisted keys
STORAGE_HITS=$(grep -rn --include="*.ts" --include="*.tsx" --exclude="*.test.*" -E 'localStorage\.setItem\(' "$FRONTEND_SRC" 2>/dev/null | grep -v "privacy-ok" || true)
